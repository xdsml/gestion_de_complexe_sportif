...
if (isset($_POST["boutton"])) {
    try {
        ...
        $query4 = "SELECT Idabonnement FROM abonnement WHERE Duree = :abonnement AND Typeabonnement = :premieum";
        $stmt4 = $conn->prepare($query4);
        $stmt4->bindParam(':abonnement', $abonnement);
        $stmt4->bindParam(':premieum', $premieum);
        $stmt4->execute();
        ...
        // Préparer et exécuter la requête pour obtenir tous les Idactivite
        $query5 = "SELECT Idactivite FROM activite";
        ...
        if ($result) {
            $activite_ids = array_column($result, 'Idactivite');
            // Combiner les IDs dans une seule variable séparée par des virgules
            $combined_activities = implode(',', $activite_ids);
