...
$activite1 = $_SESSION['activite1'];
$activite2 = $_SESSION['activite2'];
$standard = "standard";
print_r($activite1);
print_r($activite2);
...
if (isset($_POST["boutton"])) {
    try {
        ...
        //PDO li hbltoni 3liha hiya wahd l'extension PHP li katkhlina n accedew l la Base de donne
        //ou hadik new bach n instanciyew  
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        //Hadik PDO::ATTR_ERRMODE hiya li katkhli les erreur ki yetgeraw
        //ou hadik PDO::ERRMODE_EXCEPTION hiya li katkhli PDO y lancer les exception min ykoun 3ndna erreur

        // Préparer et exécuter la requête pour obtenir l'Idabonnement
        $query4 = "SELECT Idabonnement FROM abonnement WHERE Duree = :abonnement AND Typeabonnement = :standard";
        $stmt4 = $conn->prepare($query4);
        //$conn houwa lobjet li stokina fih requette
        //hna hadik prepare hiya wahd l methode li ka tprepari wahd la requet bach texecuta ye3ni katsift wahd lobjet l PHPStatement li mn moraha katexecuter la requette b les parametres dyawlha
        $stmt4->bindParam(':abonnement', $abonnement);
        $stmt4->bindParam(':standard', $standard);
        //$stmt houwa lobjet li stokina fih $conn li fih requette
        //hadik bindParam hiya li katlier les variables PHP li ana recupirithom m3a les paramettres li f requette
        $stmt4->execute();
        //ou execute raha bayna hiya li kat execute la requette preparer 

        // Récupérer le résultat de la requête
        $result = $stmt4->fetch(PDO::FETCH_ASSOC);
        //lfetch ka trecuperer une ligne mn le resulta ou fetchall katrecuperer ga3 les ligns okkkkkkk
        //hadik fetch:assoc katreturni 3la chkl tableau hit ana les id recuperethom kmlin mfhom

        // Vérifiez si un abonnement a été trouvé
        if ($result) {
            $ab = $result['Idabonnement'];
            //hna recuperet les id dkholthom f $ab
            echo "Idabonnement: " . $ab;
        } else {
            echo "Aucun abonnement trouvé pour les critères spécifiés.";
        }

        // Préparer et exécuter la requête pour obtenir les Idactivite
        $query5 = "SELECT Idactivite FROM activite WHERE Nomactivite = :activite1 OR Nomactivite = :activite2";
        $stmt5 = $conn->prepare($query5);
        $stmt5->bindParam(':activite1', $activite1);
        $stmt5->bindParam(':activite2', $activite2);
        $stmt5->execute();
        ...
        // Vérifiez si des activités ont été trouvées
        if ($result) {
            if (count($result) == 2) {
                $ac1 = $result[0]['Idactivite'];
                $ac2 = $result[1]['Idactivite'];
                
                //hna kol activite dkholtha f variable bach n inserihoum f id wahd li houwa foreign key li f abonne
                // Combiner les deux IDs dans une seule variable
                $combined_activities = $ac1 . ',' . $ac2;
                //hna hchit..
                echo "Idactivite1 (combiné): " . $combined_activities;
                
            } else {
                echo "Nombre d'activités trouvées ne correspond pas à 2.";
            }
        } else {
        ...
        $query2 = "INSERT INTO abonne (Nom, Prenom, Email, Adresse, CIN, Datenaiss, Tel, Password, Idabonnement, Idactivite) 
                   VALUES (:firstname, :lastname, :email, :adresse, :cin, :datedeN, :tel, :password, :ab, :combined_activities)";
        ...
        $stmt2->bindValue(':combined_activities', $combined_activities);
        $stmt2->execute();

        // Récupération de l'ID généré pour l'abonné
        $idAbonne = $conn->lastInsertId();
        //hadi asidi lastinsertid hiya wahd la methode li ka tretourni l id nta3 aakhir insertion dart
...
//*****************************outline hiya l contoure dyal chi element kima kan div wla mhm hadi fikhatr souhaila tb9a thbl */
//ou lfer9 mabin empty ou isset     empty hiya kat verifie wach la variable est vide ou isset katveriie wach variable mdefiniya ou machi null bsssha 
...
<div class="container">
    ...
    <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
        ...
        <input type="submit" value="Proceed to checkout"name="boutton"  class="submit-btn">
